...
本目录存放面向 sing-box 1.15.x 兼容处理后的模板配置。
...
- 废弃并移除 TUN 入站中的 `stack` 参数，默认使用 sing-tun 自有的全新原生 TCP/IP stack，在极限性能、能效和内存占用上显著优于旧实现
- 移除 TUN 入站中已废弃且不再生效的 `endpoint_independent_nat`
- 沿用 1.14.x 确立的最佳实践：已移除 `dns.independent_cache`，启用 `dns.optimistic`、`experimental.cache_file.store_dns` 与 `store_fakeip`
- TUN 模板显式设置 `dns_mode: hijack`，对齐 TUN DNS 行为
- 远程规则集统一使用 `http_client` 配合 `http_clients` 与 `route.default_http_client`，走 jsDelivr GitHub CDN 直连下载
- `clash_api.external_ui_download_url` 配置为 GitHub 加速代理源，并移除已废弃的 `external_ui_download_detour`
